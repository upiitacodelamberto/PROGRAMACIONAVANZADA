...
UT_Directory::UT_Directory():size(0),capacity(1){
	the_directory=new Directory_entry[1];
}

UT_Directory::~UT_Directory(){
...
*/
void UT_Directory::load_data(const std::string& source_name){
...
*/
std::string UT_Directory::add_or_change_entry(const std::string& var_name,const std::string& coeffs)
{
	string old_coeffs="";
...
*/
void UT_Directory::add(const std::string& var_name,const std::string& coeffs){
...
std::string UT_Directory::lookup_entry(const std::string& var_name) const {
	int index=find(var_name);
	if(index!=-1){
...
int UT_Directory::find(const std::string& var_name) const {
	for(int i=0;i<size;i++){
...
/** Obtener el n\'umero de entradas en el directorio. */
int UT_Directory::get_size(){
  return size;
}

/** Obtener la entrada index del directorio  */
Directory_entry UT_Directory::get_entry(int i){
  return the_directory[i];
}
