...
int first_and_last_are(char buf[TAMDBUF],char c1,char c2);
...
void do_multiplicacion(Pol_Directory& the_directory,string& op1,string& op2,string& var){
	...
	Polinomio PolResult=PolOp1*PolOp2;
	string resultado=PolResult.string_show();
	the_directory.add_or_change_entry(string_pol_mult,resultado);
}

int first_and_last_are(char buf[TAMDBUF],char c1,char c2){
  string strAftEq=get_string_after_equal(buf);//string After Equal
  int fd=strAftEq.find_first_not_of(" ");
  strAftEq=strAftEq.substr(fd);
  int i=strAftEq.size()-1;
  if(strAftEq[0]==c1){
    /*del final hacia el inicio, encontrar el 
      primer caracter que no es un espacio en blanco*/
    while((strAftEq[i]==' ')&&(i>0)){
      i--;
    }
    if(i==0){
      /*al parecer despu\'es del '=' en buf no hay algo*/
      return 0;
    }else{
    if(strAftEq[i]==c2)
      return 1;
