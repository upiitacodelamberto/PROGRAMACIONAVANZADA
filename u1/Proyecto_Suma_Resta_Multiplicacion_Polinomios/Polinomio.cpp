...
Polinomio& Polinomio::operator-(Polinomio& PolObj){
    Polinomio& P=PolObj*(-1);
    Polinomio RPol(*this+P);
    Polinomio& ResPol=RPol;
	return ResPol;
}
...
Polinomio& Polinomio::operator*(int factor){
  Polinomio *ResP=new Polinomio(grado,new Rac[grado+1]);
  for(int i=0;i<grado+1;i++){
    (ResP->A+i)->n=factor*((A+i)->n);
    (ResP->A+i)->d=(A+i)->d;
  }
...
Polinomio::Polinomio(Polinomio& P):grado(P.grado){
  A=new Rac[P.grado+1];
