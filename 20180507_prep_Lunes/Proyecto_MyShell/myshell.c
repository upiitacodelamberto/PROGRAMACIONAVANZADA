...
int
hay_equal_in_buf(char buf[TAMDBUF],int tamdbuf)
{
  int r=0;
  for(int i=0;i<tamdbuf;i++){
    if(buf[i]=='\n'){
      break;
    }
    if(buf[i]=='='){
...
//		if(!CreateProcess(L"C:/WINDOWS/notepad.exe",L"notepad.exe c:/readme.txt",0,0,0,0,0,0,&si,&pi))
	...
	//201805071348
    //2018.05.07.13.48	
	//Determinar si hay un '=' en el arreglo buf
    if(hay_equal_in_buf(buf,sizeof(buf))){
      cout<<"SE ENCONTRO UN \'=\' en la orden"<<endl;
    }	
